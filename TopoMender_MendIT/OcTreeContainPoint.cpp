...
void OcTree::CheckPoint(UINT64 n64CheckPoint)
{
	UCHAR * pNodeFace[2];
	UCHAR * pNodeEdge[2][2];
	...
	int nEdge[2][2];
	int nPoint[2][2][2];
	RecCheckPointCell(m_pInfiniteNode, m_n64Infinite, -1, n64CheckPoint);
	RecCheckPointCell(m_pRoot, 0, 0, n64CheckPoint);
	for (int i = 0; i < 2; i++)
	{
		...
		n64Face[i] = 0;
		nFace[i] = 0;
		RecCheckPointFace(pNodeFace, n64Face, nFace, 0, n64CheckPoint);
		RecCheckPointFace(pNodeFace, n64Face, nFace, 1, n64CheckPoint);
		RecCheckPointFace(pNodeFace, n64Face, nFace, 2, n64CheckPoint);
	}
	for (int i = 0; i < 2; i++)
		for (int j = 0; j < 2; j++)
		{
			for (int ii = 0; ii < 2; ii++)
				...
				n64Edge[i][j] = 0;
				nEdge[i][j] = 0;
				RecCheckPointEdge(pNodeEdge, n64Edge, nEdge, 0, n64CheckPoint);
				RecCheckPointEdge(pNodeEdge, n64Edge, nEdge, 1, n64CheckPoint);
				RecCheckPointEdge(pNodeEdge, n64Edge, nEdge, 2, n64CheckPoint);
		}
		for (int i = 0; i < 2; i++)
			for (int j = 0; j < 2; j++)
				for (int k = 0; k < 2; k++)
				{
					for (int ii = 0; ii < 2; ii++)
						for (int jj = 0; jj < 2; jj++)
							...
							n64Point[i][j][k] = 0;
							nPoint[i][j][k] = 0;
							RecCheckPointPoint(pNodePoint, n64Point, nPoint, n64CheckPoint);
				}
}

void OcTree::RecCheckPointCell(UCHAR * pNode, UINT64 n64Position, int nLevel, UINT64 n64CheckPoint)
{
	if (!CheckPointForCell(n64Position, nLevel, n64CheckPoint))
		return;
		...
		for (int i = 0; i < 2; i++)
			for (int j = 0; j < 2; j++)
				for (int k = 0; k < 2; k++) {
					...
					nCell = nPoint[i][j][k];
					RecCheckPointCell(pNodeCell, n64Cell, nCell, n64CheckPoint);
				}
				for (int i = 0; i < 2; i++)
					for (int j = 0; j < 2; j++)
					{
...
void OcTree::RecCheckPointFace(UCHAR * pNode[2], UINT64 n64Position[2], int nLevel[2], int dirFace, UINT64 n64CheckPoint)
{
	UINT64 n64ThisFace;
	int nThisFace;
	GetFacePosition(n64Position, nLevel, dirFace, n64ThisFace, nThisFace);
	if (!CheckPointForFace(n64ThisFace, nThisFace, dirFace, n64CheckPoint))
		return;
...
void OcTree::RecCheckPointEdge(UCHAR * pNode[2][2], UINT64 n64Position[2][2], int nLevel[2][2], int dirEdge, UINT64 n64CheckPoint)
{
	UINT64 n64ThisEdge;
	int nThisEdge;
	GetEdgePosition(n64Position, nLevel, dirEdge, n64ThisEdge, nThisEdge);
	if (!CheckPointForEdge(n64ThisEdge, nThisEdge, dirEdge, n64CheckPoint))
		return;
...
void OcTree::RecCheckPointPoint(UCHAR * pNode[2][2][2], UINT64 n64Position[2][2][2], int nLevel[2][2][2], UINT64 n64CheckPoint)
{
	UINT64 n64ThisPoint;
	GetPointPosition(n64Position, nLevel, n64ThisPoint);
	if (!CheckPointForPoint(n64ThisPoint, n64CheckPoint))
		return;
